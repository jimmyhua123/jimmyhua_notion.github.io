...
# $$W' = m \cdot (V + \Delta V)$$ 的隱含因素解析

在該公式中，雖然 $$\frac{1}{||W_0||c}$$ ， $$\frac{1}{||V + \Delta V||c}$$ 
並未顯式出現，但這並不意味著它們被省略或簡化，而是因為以下兩個關鍵原因：

### 1️⃣ $$V$$ 已包含 $$\frac{1}{||W_0||c}$$ 的影響

- $$\frac{1}{||W_0||c}$$ 在計算 $$V$$ 時已經應用，因為 $V$ 是單位方向向量矩陣。
  - 這表示在計算 $$V$$ 的過程中，已經對原始權重 $W_0$ 進行了規範化，使其具有單位長度。

- 因此，在公式 $$W' = m \cdot (V + \Delta V)$$ 中無需再額外引入 $$\frac{1}{||W_0||c}$$，因為 $$V$$ 本身已經具備這個性質。
### 2️⃣ $$\frac{1}{||V + \Delta V||c}$$ 被視為常數，避免額外計算

- 在實際計算過程中，$$\frac{1}{||V + \Delta V||c}$$ 被視為常數，並從梯度計算中分離，這樣做的主要目的是：
  - 減少訓練時的記憶體消耗

  - 避免不必要的額外運算，提高計算效率


- 雖然這個項並未直接體現在數學公式中，但它 仍然隱含在 $$V + \Delta V$$ 的計算之中。
