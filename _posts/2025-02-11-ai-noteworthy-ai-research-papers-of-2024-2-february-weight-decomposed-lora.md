...
在該公式中，雖然 $$\frac{1}{||W_0||c}$$ ， $$\frac{1}{||V + \Delta V||c}$$ 並未顯式出現，但這並不意味著它們被省略或簡化，而是因為以下兩個關鍵原因：

-  $$\frac{1}{||W_0||c}$$ 在計算 $$V$$ 時已經應用 $$V$$ 是單位方向向量矩陣，這表示在計算 $$V$$ 的過程中，已經對原始權重 $$W_0$$ 進行了規範化，使其具有單位長度。
