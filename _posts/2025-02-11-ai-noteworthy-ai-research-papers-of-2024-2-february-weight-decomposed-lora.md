...
- 在該公式中，雖然 **1/||W₀||c** 和 **1/||V + ΔV||c** 並未顯式出現，但這並不意味著它們被省略或簡化，而是因為以下兩個關鍵原因：

### **1️⃣ 1/||W₀||c 在計算 V 時已經應用**

- **V 是單位方向向量矩陣**，這表示在計算 **V** 的過程中，已經對原始權重 **W₀** 進行了規範化，使其具有單位長度。
- 因此，在公式 W' = m * (V + ΔV) 中，不需要再額外引入 **1/||W₀||c**，因為 V 已經具備這個性質。

### **2️⃣ 1/||V + ΔV||c 在梯度計算時被視為常數**

- 在實際計算過程中，**1/||V + ΔV||c 被視為常數，並從梯度圖中分離**，這樣做的目的是減少訓練時的記憶體消耗。
- 但這個項仍然隱含在 **V + ΔV** 的計算之中，而非直接的數學運算。
